...
class PDHGLayerNorm(MessagePassing):
    def __init__(self, node_dim, 
                       edge_dim, 
                       out_dim,
                       residual_dim,
        ...
        super().__init__(aggr='sum')
        '''functions for the equations'''
        self.f_edge_up = LinearAndNorm(edge_dim, out_dim)
        self.f_edge_agg = LinearAndNorm(node_dim, out_dim)
        ...
        self.residual_linear_layer = LinearAndNorm(residual_dim, out_dim)
        self.nf_linear_layer = LinearAndNorm(node_dim, out_dim)
        self.aggregation_linear_layer = LinearAndNorm(out_dim, out_dim)
...
class GraphPDHGNetNorm(nn.Module):
    """
    Small network that stacks multiple GraphConv layers.
